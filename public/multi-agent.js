...
function handleFbModeChip(next) {
  ...
  if (next === "relay" || next === "discuss") {
    ...
    const ans = prompt("最大轮数(1-30):", String(curRounds));
    const n = Math.max(1, Math.min(30, parseInt(ans || curRounds, 10) || curRounds));
    fb.setMaxRounds(n);
    fb.start();
  }
  renderAll();
...
function renderFishbowlSidePanel() {
  ...
  if (mode === "relay" || mode === "discuss") {
    if (s.state === "ended" && s.endStats) {
      ...
      statusHtml = `<div class="fb-side-status"><div class="fb-state-line">${stateLabel}</div><div class="fb-info-line">轮次 <b>${round}</b></div>${topic ? `<div class="fb-info-line">议题:${safe(topic)}</div>` : ""}<div class="fb-info-line">当前:<b>${safe(speaker)}</b></div></div>`;
    }
  }
