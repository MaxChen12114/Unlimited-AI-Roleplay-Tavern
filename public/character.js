...
function render(){
  ...
  const ei=P.querySelector("[data-ec]");
  if(ei)ei.addEventListener("change",()=>{setEmo(ei.value||"neutral");render();});
...
function editor(b,card){
  ...
  b.innerHTML=`<div class="char-edit-form">
    ...
    <label class="char-label">性别</label><div class="char-relation-row"><button class="char-rel-btn ${c.gender==="female"?"active":""}" data-g="female" type="button">女</button><button class="char-rel-btn ${c.gender==="male"?"active":""}" data-g="male" type="button">男</button></div>
    ...
    <label class="char-label" style="display:flex;align-items:center;gap:8px;cursor:pointer;font-weight:normal;"><input type="checkbox" data-f="enableAff" ${c.enableAffection===false?"":"checked"}><span>启用好感度系统（初始 ${AFF_INIT}，AI 在回复末尾用 [好感±N] 调整）</span></label>
    <label class="char-label">好感度阈值事件（跨过分值时一次性触发剧情指令 · 重置好感度会清空触发记录）</label>
    <div data-thresholds-wrap>${(c.affectionThresholds||[]).map((th,i)=>thresholdRowHtml(th,i)).join("")}</div>
    <button class="char-rel-btn" type="button" data-add-threshold style="font-size:11px;padding:3px 10px;align-self:flex-start;">+ 添加阈值事件</button>
  ...
  let gen=c.gender;
  b.querySelectorAll("[data-g]").forEach(x=>x.addEventListener("click",()=>{gen=x.dataset.g;b.querySelectorAll("[data-g]").forEach(y=>y.classList.toggle("active",y.dataset.g===gen));}));
